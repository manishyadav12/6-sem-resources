(function ($) {
  ...
  $.fn.barChart = function (options) {
    ...
    $(this)
      .css(
        "height",
        settings.height && !settings.vertical ? settings.height : "auto"
      )
      .addClass("bar-chart")
  ...
  // init
  function init() {
    settings.maxHeight = settings.vertical
      ? $(this).width()
      : $(this).height() - settings.totalSumHeight;

    settings.maxWidth = settings.vertical
  ...
  function drawY(columns) {
    ...
    max = settings.vertical
      ? Math.ceil(max)
      : Math.ceil(max / multiplier) * multiplier;
  ...
  function drawX(columns) {
    ...
    var columnWidth = Math.round(
      (settings.maxWidth - settings.barGap * (columnsCount + 1)) / columnsCount
    );
    ...
    if (!settings.vertical) {
      if (columnWidth < settings.defaultColumnWidth) {
        //settings.defaultColumnWidth = 65
        ...
        var $bar = $("<div />")
          ...
          .css(
            settings.vertical
              ...
              : {
                  width: columnWidth + "%",
                  marginLeft: settings.barGapPercent + "%",
                }
          )
          .attr({ "data-id": key })
          .append($barValue)
