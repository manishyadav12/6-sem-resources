(function () {
  ...
  var Maps = function (elt) {
    ...
    var _sources = {
      nations:
        'Derived from Wikipedia’s <a target="_blank" href="http://en.wikipedia.org/wiki/List_of_countries_and_territories_by_land_borders">List of countries and territories by land borders</a>',
      states:
        'Derived from <a target="_blank" href="http://www.statemaster.com/graph/geo_lan_bou_bor_cou-geography-land-borders">Land borders by state</a>',
      ...
      selectMap: function (map_id) {
        $.getJSON("maps/" + map_id + ".json", function (data) {
          ...
          $.each(nodes, function (name, info) {
            info.label = name
              .replace(/(people's )?republic of /i, "")
              .replace(/ and /g, " & ");
          });
